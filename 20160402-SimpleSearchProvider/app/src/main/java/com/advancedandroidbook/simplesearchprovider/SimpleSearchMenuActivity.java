...
public class SimpleSearchMenuActivity extends Activity {
	...
	private void deleteAllFieldnotes() {
		getContentResolver().delete(SimpleFieldnotesContentProvider.CONTENT_URI, null, null);
		/*
		getContentResolver().delete(
				Uri.parse("content://com.advancedandroidbook.simplesearchprovider.SimpleFieldnotesContentProvider/fieldnotes_provider"),
				null,
				null
		);
		*/
	...
	private void addFieldnote(String headline, String story) {
		...
		getContentResolver().insert(SimpleFieldnotesContentProvider.CONTENT_URI, values);
	}
	...
	public void onLaunchGlobalSearchSettings(View view) {
		...
		if (intent.resolveActivity(getPackageManager()) != null) {
			...
			Toast.makeText(getApplicationContext(), "The emulator does not have access to Google Play for Global Search settings", Toast.LENGTH_SHORT).show();
		}
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			...
			searchView.setSearchableInfo(searchManager.getSearchableInfo(new ComponentName(this, SimpleSearchableActivity.class)));
