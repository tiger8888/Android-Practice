...
public class SimpleFieldnotesContentProvider extends ContentProvider {
	...
	public static final String CONTENT_ITEM_TYPE = ContentResolver.CURSOR_ITEM_BASE_TYPE + "/vnd.advancedandroidbook.search." + BASE_DATA_NAME;
	public static final String CONTENT_TYPE = ContentResolver.CURSOR_DIR_BASE_TYPE + "/vnd.advancedandroidbook.search." + BASE_DATA_NAME;
	
	// common URIs
	// CONTENT_URI = "content://com.advancedandroidbook.simplesearchprovider.SimpleFieldnotesContentProvider/fieldnotes_provider";
	public static final Uri CONTENT_URI = Uri.parse("content://" + AUTHORITY + "/" + BASE_DATA_NAME);
	// SEARCH_SUGGEST_URI = "content://com.advancedandroidbook.simplesearchprovider.SimpleFieldnotesContentProvider/fieldnotes_provider/" + SearchManager.SUGGEST_URI_PATH_QUERY;
	...
	static {
		...
		sURIMatcher.addURI(AUTHORITY, BASE_DATA_NAME + "/" + SearchManager.SUGGEST_URI_PATH_QUERY, FIELDNOTES_SEARCH_SUGGEST);
		sURIMatcher.addURI(AUTHORITY, BASE_DATA_NAME + "/" + SearchManager.SUGGEST_URI_PATH_QUERY + "/*", FIELDNOTES_SEARCH_SUGGEST);
	}
	...
	static {
		...
		FIELDNOTES_SEARCH_SUGGEST_PROJECTION_MAP.put(SearchManager.SUGGEST_COLUMN_TEXT_1, FIELDNOTES_TITLE + " AS " + SearchManager.SUGGEST_COLUMN_TEXT_1);
		...
		// FIELDNOTES_BODY);
		FIELDNOTES_SEARCH_SUGGEST_PROJECTION_MAP.put(SearchManager.SUGGEST_COLUMN_INTENT_DATA_ID, _ID + " AS " + SearchManager.SUGGEST_COLUMN_INTENT_DATA_ID);
	}
	// the database
	...
	public int delete(Uri uri, String whereClause, String[] whereArgs) {
		...
		switch (match) {
			case FIELDNOTES:
				rowsAffected = sql.delete(SimpleFieldnotesDatabase.FIELDNOTES_TABLE, whereClause, whereArgs);
				break;
			case FIELDNOTE_ITEM:
				String id = uri.getLastPathSegment();
				if (TextUtils.isEmpty(whereClause)) {
					rowsAffected = sql.delete(SimpleFieldnotesDatabase.FIELDNOTES_TABLE, _ID + "=" + id, null);
				} else {
					rowsAffected = sql.delete(SimpleFieldnotesDatabase.FIELDNOTES_TABLE, whereClause + " and " + _ID + "=" + id, whereArgs);
				}
				break;
			default:
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		if (match == FIELDNOTES) {
			SQLiteDatabase sql = database.getWritableDatabase();
			long newId = sql.insert(SimpleFieldnotesDatabase.FIELDNOTES_TABLE, null, values);
			if (newId > 0) {
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		switch (match) {
			case FIELDNOTES_SEARCH_SUGGEST:
				...
				queryBuilder.setProjectionMap(FIELDNOTES_SEARCH_SUGGEST_PROJECTION_MAP);
				break;
			case FIELDNOTES:
				break;
			case FIELDNOTE_ITEM:
		...
		Cursor cursor = queryBuilder.query(sql, projection, selection, selectionArgs, null, null, sortOrder);
	...
	private class SimpleFieldnotesDatabase extends SQLiteOpenHelper {
		...
		private static final String FIELDNOTES_SCHEMA = "CREATE TABLE "
				...
				+ ");";
		private static final String UPGRADE_DB_SCHEMA = "DROP TABLE IF EXISTS " + FIELDNOTES_TABLE;
